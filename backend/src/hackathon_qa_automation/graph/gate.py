...
def decide(lead_id: str, verdicts: list[CheckVerdict],
    ...
    critical_fails = [v.check_name for v in verdicts if v.critical and v.verdict == "fail"
                      and v.confidence >= CONFIDENCE_THRESHOLD]
    if critical_fails:
        return _decided(lead_id, "HOLD", [f"Critical check failed: {c}" for c in critical_fails])

    unresolved = [v.check_name for v in verdicts
                  if v.verdict == "uncertain" or v.confidence < CONFIDENCE_THRESHOLD]
    if unresolved:
        return _decided(lead_id, "HUMAN_QA",
                        [f"Unclear or low-confidence result: {c}" for c in unresolved])
