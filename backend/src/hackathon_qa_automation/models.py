...
class Check(BaseModel):
    check_id: str
    name: str = ""  # short label shown to people; defaults to a tidied check_id
    description: str = ""
    type: CheckType
    critical: bool = False
    required_elements: list[str] = Field(default_factory=list)  # verbatim
    spoken_by: Literal["AGENT", "CUSTOMER"] = "AGENT"  # verbatim: CUSTOMER = must agree in reply
    ...
    def _validate(self) -> "Check":
        ...
        if not self.name:
            self.name = self.check_id.replace("_", " ").capitalize()
        ...
        if self.spoken_by == "CUSTOMER" and self.type != "verbatim":
            raise ValueError(f"Check {self.check_id!r}: spoken_by CUSTOMER is only for verbatim checks")
